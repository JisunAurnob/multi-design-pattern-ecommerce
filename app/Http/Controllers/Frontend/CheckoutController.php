...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('frontend.pages.checkout', compact('total_discount', 'grand_total', 'shipping_charge',
            'total', 'carts'));
    }
...
//dd($order);
            ...
            //     dispatch(new OrderMailJob($order->shipping_email,$order));
            // }

            // if(!empty($order->shipping_phone)){
            //     sendSms($order->shipping_phone,"Order placed!!\nName: $order->shipping_name\nOrder-ID: $order->order_number\nTotal amount: $order->pay_amount\nFor any assistance please contact: 09613664444 (Ultimate Organic Life)");
            // }

            // if(!empty($order->customer->customer_contact)){
            //     sendSms($order->customer->customer_contact,"Your order (Order-ID: $order->order_number) at Ultimate Organic Life is ". $order->status);
            // }
